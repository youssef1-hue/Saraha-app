...
import { get, set, otpKey, ttl, incr, maxAttempOtpKey, deleteKey, blockOtpKey, keys, baseRevonTokenKey } from "../../common/services/redis.service.js";
...
const sendEmailOtp = async ({ email, subject, title } = {}) => {
  ...
  if (isBloked > 0) {
    throw badRequestException({
      message: `You have exceeded the maximum number of OTP attempts. Please try again after ${isBloked} seconds.`,
    });
  }
  ...
  if (remainingOtpTTL > 0) {
    throw badRequestException({
      message: `Please wait ${remainingOtpTTL} seconds before requesting a new OTP`,
    });
  }
  ...
  if (maxTrial >= 3) {
    ...
    throw badRequestException({
      message: "You have exceeded the maximum number of OTP attempts. Please try again after 7 minutes.",
    });
  }
...
export const signup = async (inputs) => {
  const { email, password, phone, firstName, lastName } = inputs;
...
const verifyGoogleAccount = async (idToken) => {
  ...
  if (!payload) {
    throw badRequestException({ message: "Failed to verify Google account" });
  }
...
export const signupWithGmail = async (idToken, issuer) => {
  ...
  return {
    status: 201,
    credentials: createLoginCredentials(user, issuer),
  };
};
